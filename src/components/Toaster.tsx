...
export const Toaster = (props: ToasterPropsType) => {
    const isError = useSelector<AppStateType,boolean>(state => state.appReducer.isToasterVisible)
    const displayNone = isError ? 'visible opacity-100' : 'invisible opacity-0'
    switch (props.type) {
        case "error":{
            return (
                <div id="toast-danger"
                     className={`flex items-center w-full max-w-xs p-4 transition-opacity mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 ${displayNone}`}
                     role="alert">
                    <div
                    ...
                    <div className="ml-3 text-sm font-normal">{props.innerText}</div>
                </div>
            );
        }
        case "warning":{
